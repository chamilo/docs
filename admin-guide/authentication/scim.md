...
**SCIM** (System for Cross-domain Identity Management) automatise le provisionnement des utilisateurs — création, mise à jour et désactivation des comptes Chamilo en fonction des modifications dans votre fournisseur d’identité. Contrairement à OAuth2 ou LDAP, SCIM gère le provisionnement, et non la connexion.

| Scénario | Action SCIM |
|----------|-------------|
| Un nouvel employé arrive | Crée un compte Chamilo |
| Le nom ou le rôle d’un employé change | Met à jour le compte Chamilo |
| Un employé part | Désactive ou supprime le compte Chamilo |
...
Videz et préchauffez le cache après modification :
...
3. Saisissez le jeton de l’étape 1 comme jeton bearer
4. Mappez les attributs du fournisseur vers les champs standard SCIM (userName, name.givenName, name.familyName, emails)
...
* **Commencez par un groupe de test** — provisionnez un petit ensemble d’utilisateurs avant d’activer SCIM pour toute l’organisation.
...
* **Surveillez les journaux** — consultez à la fois Chamilo (`var/log/`) et les journaux de provisionnement de votre fournisseur d’identité pour détecter les erreurs.