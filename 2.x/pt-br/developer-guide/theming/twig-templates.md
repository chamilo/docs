...
O Chamilo utiliza Twig para páginas renderizadas no lado do servidor. Os modelos estão localizados em `src/CoreBundle/Resources/views/` e são referenciados com o prefixo de namespace `@ChamiloCore/` (por exemplo, `@ChamiloCore/Layout/base-layout.html.twig`).
...
3. O Vue é montado em `#app` e gerencia toda a renderização da interface do usuário dentro desse elemento.
4. O aplicativo Vue se comunica com o backend por meio da API REST.

Para páginas legadas que ainda não foram migradas para o Vue, o Symfony renderiza o HTML completo da página via Twig e o conteúdo é colocado dentro de `#sectionMainContent`. O Vue ainda é montado (fornecendo a barra lateral e a barra superior), mas a área de conteúdo principal é HTML renderizado no servidor.
...
| `Layout/layout_empty.html.twig` | Layout vazio com o mínimo de elementos visuais |
...
{# Em foot.html.twig — JS do Vue (carregado no final do body) #}
{{ encore_entry_script_tags('vue') }}
```

Entradas JS legadas (`legacy_app`, `legacy_lp`, etc.) são carregadas em `<head>` porque páginas PHP legadas dependem delas estarem disponíveis antes que o DOM esteja pronto.
...
Principais funções Twig disponíveis em todos os modelos (registradas em `ChamiloExtension`):
