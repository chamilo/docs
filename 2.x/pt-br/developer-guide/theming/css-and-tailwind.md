...
1. **Tailwind CSS** — Classes utilitárias para layout, espaçamento e cor. Configurado com `important: true` para que as utilidades substituam os padrões dos componentes PrimeVue.
...
4. **Tema `colors.css`** — Propriedades personalizadas CSS para o tema de cores ativo, carregado por último para que se sobreponha a tudo o mais.

PrimeFlex está listado em `package.json`, mas não é importado — o Tailwind cobre todas as necessidades de utilidades.
...
Os caminhos de conteúdo escaneiam componentes Vue, páginas PHP legadas, arquivos de plugins e templates Twig para que utilidades não utilizadas sejam removidas em builds de produção.
...
O helper `colorWithOpacity` emite `rgb(var(--color-primary-base) / <opacity>)`, permitindo variantes de opacidade como `bg-primary/50`. Os valores RGB reais são definidos por tema em `var/themes/{slug}/colors.css` e carregados em tempo de execução — veja [Temas de Cores](color-themes.md).
...
Como `important: true` está definido em `tailwind.config.js`, as utilidades do Tailwind substituem de forma confiável os estilos dos componentes PrimeVue sem a necessidade de especificidade extra.