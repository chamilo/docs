...
Gerencia todos os eventos de sala BBB. Valida a assinatura HMAC, então insere ou atualiza uma linha `ConferenceActivity` e atualiza o campo JSON de métricas.
...
2. Busca o `ConferenceMeeting` pelo `remote_id`.
3. Busca (ou cria) uma linha aberta de `ConferenceActivity` para aquela reunião + usuário.
...
- Escrever um plugin que escute eventos do Symfony e despache chamadas HTTP (consulte [Plugins](../plugins/README.md) e [Sistema de Eventos](../events.md)).
