...
O Chamilo 2.0 utiliza um grande número de controladores (na ordem de dezenas) organizados em diferentes bundles. A contagem exata varia de versão para versão — considere os nomes abaixo como ilustrativos, não exaustivos.
...
### Controladores de Ações da API

Ações personalizadas da API Platform em `src/CoreBundle/Controller/Api/`:

Esses controladores estendem o CRUD integrado da API Platform com lógica de negócios personalizada. Exemplos:

* `CreateDocumentFileAction` — Upload de arquivos para documentos
* `CreateStudentPublicationFileAction` — Upload de submissões de tarefas
...
Para operações de leitura/escrita que não necessitam de um controlador HTTP dedicado — ou seja, quando você só deseja alterar *como* um item ou coleção é buscado ou persistido — prefira um **State Provider** ou **State Processor** (veja abaixo). Os Controladores de Ações da API são mais adequados para endpoints que realmente precisam de lógica no nível da requisição (uploads de arquivos, formatos de resposta personalizados, fluxos de várias etapas).

### Controlador de IA

`src/CoreBundle/Controller/AiController.php` é o ponto de entrada para endpoints relacionados a IA (geração de questões Aiken, geração de caminhos de aprendizagem, geração de imagens/vídeos, correção de respostas abertas, análise de documentos...). O conjunto exato de rotas evolui rapidamente — leia os atributos `#[Route]` do controlador para obter a lista atual, em vez de depender de uma cópia aqui.
...
Nem todos os endpoints da API são suportados por um controlador. A API Platform 3 divide o trabalho entre duas interfaces:

* **State Providers** (`ApiPlatform\State\ProviderInterface`) — retornam dados para operações `GET` (um único item ou uma coleção).
* **State Processors** (`ApiPlatform\State\ProcessorInterface`) — gerenciam operações de escrita para `POST`, `PUT`, `PATCH` e `DELETE`.

As implementações do Chamilo estão em `src/CoreBundle/State/` (cerca de 35+ classes). Elas são vinculadas às entidades por meio dos argumentos `provider:` e `processor:` das operações `#[ApiResource]`, em vez de por meio de rotas.

### Quando usá-los

Opte por um provedor/processador — em vez de um Controlador de Ações da API — quando:
...
* Você precisa executar efeitos colaterais em operações de escrita (logs de auditoria, geração de arquivos, atualizações de entidades relacionadas) enquanto mantém o pipeline de normalização, validação e paginação da API Platform.
* Você deseja manter a operação detectável no esquema OpenAPI / Hydra sem registrar uma rota personalizada.

Se o endpoint, por outro lado, precisar de acesso bruto ao `Request`, retornar um payload que não seja um recurso (download de arquivo, CSV, redirecionamento) ou orquestrar um fluxo de várias etapas, um Controlador de Ações da API em `src/CoreBundle/Controller/Api/` é uma escolha mais adequada.

### Vinculação na entidade
...
`src/CoreBundle/State/DocumentProvider.php` resolve um `CDocument` por variável de URI e lança `NotFoundHttpException` quando não encontrado:
...
`src/CoreBundle/State/ColorThemeStateProcessor.php` delega ao `persistProcessor` padrão do Doctrine e, em seguida, executa efeitos colaterais (gera um arquivo CSS no sistema de arquivos Flysystem de temas, vincula o tema à URL de Acesso atual):
...
final readonly class ColorThemeStateProcessor implements ProcessorInterface
{
    ...
    public function process($data, Operation $operation, array $uriVariables = [], array $context = []): ?ColorTheme
    {
        ...
        // …gera colors.css, vincula à AccessUrl atual, executa flush…
