...
A família `Base*` em `assets/vue/components/basecomponents/` encapsula os primitivos do PrimeVue com padrões específicos do Chamilo (layout FloatLabel, ícones MDI via `chamiloIconToClass`, mensagens de validação consistentes, dimensionamento com Tailwind). Sempre opte por um componente `Base*` antes de importar o componente subjacente do PrimeVue — é assim que a interface do usuário mantém a consistência em toda a SPA e como mudanças de design podem ser implementadas a partir de um único local.
...
A maioria aceita o valor por meio de `v-model`, expõe as propriedades `id` + `label` para acessibilidade/vinculação de rótulo flutuante e apresenta validação por meio de um par `isInvalid` / `errorText` (ou `messageText`).

| Componente                        | Envolve                                              | Finalidade                                                                                                                                                                                            |
|-----------------------------------|------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `BaseInputText.vue`              | `InputText` + `FloatLabel`                           | Campo de texto de linha única. Muda para um rótulo estático em entradas do tipo `date`/`time`/`datetime-local` (onde o rótulo flutuante sobreporia o placeholder nativo).                           |
...
| `BaseInputTags.vue`              | (personalizado)                                      | Etiquetas de texto livre; as etiquetas são adicionadas ao pressionar enter/vírgula e removidas ao usar backspace.                                                                                     |
| `BaseInputGroup.vue`             | `InputGroup` + `BaseButton`                          | Campo de texto combinado com um botão de ação (estilo de busca).                                                                                                                                      |
...
| `BaseToggleButton.vue`           | `BaseButton`                                         | Botão de dois estados (rótulos e ícones de ligado/desligado) vinculado por meio de `v-model`.                                                                                                         |
| `BaseCalendar.vue`               | `DatePicker` + `FloatLabel`                          | Seletor de data/data-hora. Respeita `platform.timepicker_increment` e a localidade do usuário por meio de `calendarLocales`.                                                                          |
| `BaseColorPicker.vue`            | nativo `<input type="color">` + `InputText`          | Seletor de cores com fallback para texto hexadecimal; utiliza `colorjs.io` para validar entrada manual de hexadecimal.                                                                               |
...
| `BaseUploader.vue`               | Uppy `Dashboard`                                     | Carregador completo Uppy (webcam, áudio, editor de imagem, upload XHR) com localizações vinculadas ao `appLocale` atual. Use para uploads ricos com progresso; use `BaseFileUpload*` para anexos simples. |
...
| `BaseUserFinder.vue`    | `BaseTable` + `userService`    | Tabela de pesquisa de usuários paginada com seleção de linhas. Use quando uma funcionalidade precisar de um seletor de usuários no estilo administrador. |

### Botões e ações

| Componente                        | Envolve               | Finalidade                                                                                                                                                                                                                                                                                                                                                     |
|-----------------------------------|-----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `BaseButton.vue`                  | `Button` (PrimeVue)   | Botão padrão do Chamilo. Resolve ícones por meio de `chamiloIconToClass`, normaliza `type` para `severity`/`variant` do PrimeVue, renderiza um `BaseAppLink` interno quando um `route` ou `toUrl` é fornecido (assim, o mesmo componente lida com casos de router-link, âncora e botão simples). Os valores aceitos para `type` estão listados em `validators.js` → `buttonTypeValidator`. |
...
| `BaseToolbar.vue`                 | `Toolbar`             | Barra de ferramentas de ação com slots `start` / `end` (ou um único slot padrão). Opcional `showTopBorder` para estilização de separador.                                                                                                                                                                                                                     |
...
| Componente            | Envolve                     | Finalidade                                                                                                                                                                                         |
|-----------------------|-----------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `BaseTable.vue`       | `DataTable` (PrimeVue)      | Tabela de dados padrão do Chamilo. Suporta modo do lado do servidor (`lazy`), ordenação por múltiplas colunas, filtro global, seleção de linhas e paginação. Passe colunas como filhos `<Column>` (registrados globalmente). |
...
| Componente                 | Envolve                 | Finalidade                                                                                                                                                                                 |
|----------------------------|-------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `BaseMenu.vue`             | `Menu` (popup)          | Menu popup que entende rotas do roteador dentro de itens `model[]`.                                                                                                                        |
...
| `BaseRouteTabs.vue`        | Linha de `BaseAppLink`  | Barra de abas onde cada aba é um link de roteador. A aba ativa é destacada automaticamente com base na rota atual.                                                                        |
...
|-----------|-----------|---------|
| `course/` | Cartões de curso, filtros de catálogo, formulários de curso | Listagem e gerenciamento de cursos |
...
| `filemanager/` | CourseDocuments, PersonalFiles | Navegador e gerenciamento de arquivos |
| `installer/` | Step1-Step7, EmailSettings | Assistente de instalação |
| `social/` | GroupInfoCard, postagens sociais | Recursos de rede social |
| `attendance/` | AttendanceTable | Rastreamento de presença |
| `usergroup/` | GroupMembers | Gerenciamento de grupos de usuários |
...
* **Integração com PrimeVue** — Uso intenso de componentes PrimeVue (Button, DataTable, Dialog, Menu, etc.)
* **Axios para chamadas de API** — Requisições HTTP para a API do backend
* **Vue I18n** — Todo texto voltado ao usuário utiliza chaves de tradução