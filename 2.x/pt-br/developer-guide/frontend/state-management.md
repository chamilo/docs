...
O Chamilo utiliza duas bibliotecas de gerenciamento de estado lado a lado:

- **Pinia** — o padrão atual para todas as novas stores. A maior parte do código base utiliza Pinia.
- **Vuex** — store legado, ainda presente e usado por visualizações mais antigas. Novos códigos devem usar Pinia.
...
Contém configurações globais da plataforma obtidas de `/platform-config/list`:
...
| `modules/notifications.js` | Estado de notificações toast (exibir, cor, texto, tempo limite) |
