...
A seguir, descreve-se a pilha tecnológica do Chamilo 2.0. Todas as versões mencionadas aqui estão sujeitas a alterações conforme novas versões do Chamilo são lançadas. Os números de versão utilizam a [notação de versões do Composer](https://getcomposer.org/doc/articles/versions.md), que estabelece regras para permitir certa flexibilidade em relação às versões.

Incluindo dependências hierárquicas, o Chamilo utiliza centenas de bibliotecas de Software Livre. Esta lista inclui apenas aquelas que usamos com mais frequência e que provavelmente afetarão o trabalho de um desenvolvedor do Chamilo semanalmente. Somos gratos a todos os outros desenvolvedores de Software Livre que tornam nosso trabalho mais fácil, sustentável e seguro.
