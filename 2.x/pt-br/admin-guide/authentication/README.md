...
O Chamilo suporta múltiplos métodos de autenticação, desde o sistema interno de nome de usuário/senha até soluções de logon único (single sign-on) corporativas.

## Arquivo de Configuração

Todos os métodos de autenticação externos são configurados no arquivo `config/authentication.yaml`. Um modelo está disponível em `config/authentication.dist.yaml`. A estrutura geral é:
