...
Permite que tutores sempre editem anúncios dentro de sessões ativas ou passadas.
...
Permite que os gerentes de sessões configurem anúncios que serão disparados em datas específicas ou após/antes de um número de dias do início/fim da sessão. Ativar esse recurso exige a configuração de uma tarefa cron.
...
Remove a caixa de seleção para habilitar o envio de anúncios para usuários com papéis de RH (ainda requer confirmação na ferramenta de anúncios).
...
Permite que professores configurem anúncios que serão enviados em datas específicas. Isso exige a configuração de uma tarefa cron em cron/course_announcement.php executando pelo menos uma vez ao dia.
