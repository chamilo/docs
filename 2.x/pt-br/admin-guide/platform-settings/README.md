...
O Chamilo possui um extenso sistema de configuração com ajustes organizados em categorias. O conjunto completo de categorias abaixo reflete a página **Configurações de configuração** no painel de administração — e o arquivo subjacente `SettingsCurrentFixtures.php` no código-fonte, que é a fonte de verdade para nomes de variáveis, títulos e descrições.

Acesse as configurações da plataforma pelo painel de administração clicando em **Configurações de configuração**.
...
* **[Boletim (Avaliações)](gradebook-settings.md)** (34) — Exibição de pontuação, decimais, limiares para certificados.
...
* **[Busca](search-settings.md)** (3) — Configuração de busca de texto completo Xapian.
...
* As alterações entram em vigor imediatamente (não é necessário reiniciar o servidor), embora sua sessão de usuário possa manter algumas delas em memória. Se as alterações não forem refletidas imediatamente, faça logout e login novamente para limpar sua sessão.
