...
Por padrão, o Chamilo impede a edição de testes incluídos em uma trilha de aprendizagem. Isso evita alterações que afetem os alunos (passados e futuros) de maneira diferente em relação aos resultados e/ou progresso na trilha de aprendizagem. Esta opção permite que professores ignorem essa restrição.

### `allow_exercise_categories`

**Habilitar categorias de testes**

As categorias de testes não estão habilitadas por padrão, pois adicionam um nível de complexidade. Ative este recurso para que todos os ícones de gerenciamento relacionados às categorias de testes apareçam.
...
Habilita a seleção de perguntas obrigatórias em um teste ao usar categorias aleatórias.
...
Por padrão, o Chamilo permite que você mostre feedback em cada resposta de uma pergunta. Com esta opção, um campo adicional é criado para fornecer feedback predefinido para toda a pergunta. Esse feedback só aparecerá se o usuário responder incorretamente.
...
Por padrão, só é possível limitar o tempo por teste. Limitá-lo por pergunta adiciona uma camada extra de possibilidades, e você pode (com cuidado) combinar ambos.
...
Ao usar esta opção, uma opção adicional aparecerá na configuração do teste. Ao usar um teste com várias categorias de perguntas e solicitar uma distribuição por categoria, isso permitirá que o usuário navegue pelas perguntas por categoria. Uma vez que uma categoria é concluída, ele(a) avança para a próxima categoria e não pode retornar à categoria anterior.
...
**Habilitar para ignorar a configuração de Exercício invisível em sessão no nível do curso base**

Permite configurar a invisibilidade de exercícios em sessões no curso base, ignorando a configuração global. Se não estiver definido, o parâmetro global será utilizado.
...
Define se os gerentes de curso (professores) devem ser notificados por e-mail quando um questionário for respondido por um aluno. Este é o valor padrão para todos os novos cursos, mas cada professor ainda pode alterar essa configuração em seu próprio curso.
...
**Arredondar pontuação em exportações de teste**
...
**Ocultar fita de pergunta (certo/errado) nos resultados de teste**

Nos resultados de teste, uma fita aparece por padrão para indicar se a resposta estava certa ou errada. Ative esta opção para remover a fita globalmente.
...
**Máximo de editores na tela de resultado de exercício**
...
Oculta o fato de que questões abertas (incluindo áudio e anotações) possuem pontuação, escondendo a exibição da pontuação em todos os relatórios voltados para os alunos.
...
Ativa a gravação de todas as respostas (mesmo as temporárias) na tabela track_e_attempt_recording. Esta funcionalidade é experimental e pode causar problemas nas páginas de relatórios ao tentar avaliar um teste.
...
Esta opção adiciona uma caixa de seleção ao final de cada teste, pedindo ao usuário para confirmar o número de respostas salvas. Isso fornece dados de auditoria melhores para testes críticos.
...
O comportamento padrão é tornar as questões órfãs quando o único teste que as utiliza é excluído. Quando ativada, esta opção garante que todas as questões que de outra forma ficariam órfãs também sejam excluídas.
...
**Adicionar pontuação na notificação por e-mail de envio de teste**
...
Por padrão, o Chamilo não exibe uma pontuação para os tipos de pergunta de grau de certeza.
