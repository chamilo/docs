# Configurações de CAS

Configuração legada de CAS (Central Authentication Service) herdada do Chamilo 1.x. Consulte [CAS](../authentication/cas.md) para o status atual do autenticador CAS no Chamilo 2.x.
...
Ativar a autenticação CAS permitirá que os usuários se autentiquem com suas credenciais CAS.<br/>Acesse <a href='settings.php?category=CAS'>Plugin</a> para adicionar um botão configurável de 'Login CAS' para o seu campus Chamilo. Ou você pode forçar a autenticação CAS definindo cas[force_redirect] em app/config/auth.conf.php.
