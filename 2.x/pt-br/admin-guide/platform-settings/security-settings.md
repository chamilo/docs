...
Habilite esta opção para adicionar um indicador visual da força da senha quando o usuário alterar sua senha. Isso NÃO impedirá a criação de senhas fracas, apenas atua como um auxiliar visual.
...
Forneça uma lista de termos, um por linha, a serem filtrados de páginas da web e e-mails. Esses termos serão substituídos por ***.

### `force_renew_password_at_first_login`

**Forçar renovação de senha no primeiro login**

Esta é uma medida simples para aumentar a segurança do seu portal, solicitando que os usuários alterem imediatamente sua senha, de modo que a senha enviada por e-mail não seja mais válida e eles usem uma senha criada por eles mesmos, sendo a única pessoa a conhecê-la.
...
Essa configuração permite que você experimente reportando, mas sem impor algumas Políticas de Segurança de Conteúdo.

### `security_public_key_pins`

**Fixação de Chave Pública HTTP**

A Fixação de Chave Pública HTTP protege seu site contra ataques MiTM que utilizam certificados X.509 falsificados. Ao listar apenas as identidades que o navegador deve confiar, seus usuários ficam protegidos no caso de uma autoridade de certificação ser comprometida.

### `security_public_key_pins_report_only`

**Fixação de Chave Pública HTTP apenas para relatório**

Essa configuração permite que você experimente reportando, mas sem impor algumas Fixações de Chave Pública HTTP.
...
Essa opção permite gerar um token de uso único com validade limitada, enviado por e-mail ao usuário para redefinir sua senha.
