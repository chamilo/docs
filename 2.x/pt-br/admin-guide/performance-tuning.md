...
> **Referência adicional**: Sua instalação do Chamilo inclui um guia de otimização estendido. Abra `/documentation/optimization.html` em um navegador (por exemplo, `https://seu-site-chamilo/documentation/optimization.html`) para recomendações específicas de servidor para sua versão.
...
| **Carregamento adiado de JavaScript** | Carrega arquivos JavaScript não críticos de forma assíncrona para evitar bloquear a renderização da página. |
...
| Ação | Descrição |
|------|-----------|
| **Usar pool de conexão de banco de dados** | Para plataformas de alta concorrência, configure o pool de conexão para reduzir a sobrecarga de estabelecer conexões com o banco de dados. |
...
| **HTTP/2** | Habilite HTTP/2 no seu servidor web para conexões multiplexadas e compressão de cabeçalhos. |
