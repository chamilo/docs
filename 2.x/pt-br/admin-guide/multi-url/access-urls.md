...
### Habilitando Multi-URL

O recurso Multi-URL deve ser habilitado na configuração do Chamilo (geralmente nas configurações de ambiente). Isso normalmente é feito durante a configuração inicial.
...
* **Decida cedo** — Se optar por uma configuração multi-URL, você deve fazer isso no início do seu projeto Chamilo, pois isso exige que a primeira URL permaneça relativamente vazia de conteúdo. Habilitar multi-URL posteriormente é mais desafiador (requer alterações manuais no banco de dados).
* **Planeje a estrutura de URLs** — Decida sobre o esquema de URLs antes de criar URLs de acesso, pois alterar URLs posteriormente afeta todos os links e favoritos existentes
