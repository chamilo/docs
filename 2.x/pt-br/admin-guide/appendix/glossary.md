...
Termos-chave utilizados na administração do Chamilo 2.0.
...
| **Sessão** | Uma instância limitada no tempo de um ou mais cursos. As sessões permitem que o mesmo conteúdo de curso seja entregue a diferentes grupos de alunos com rastreamento separado e tutores independentes. |
...
| **Tutor/Instrutor** | Uma função no nível de sessão. Tutores de sessão supervisionam todos os cursos em uma sessão; tutores de curso gerenciam um curso específico dentro de uma sessão. Todas as referências a tutores devem ser renomeadas para instrutores a longo prazo. |
...
| **LTI** | Learning Tools Interoperability. Um padrão da IMS Global que permite que ferramentas e conteúdos externos sejam incorporados dentro de um LMS. O Chamilo suporta LTI 1.1 e 1.3 tanto como consumidor quanto como provedor. |
...
| **CAS** | Central Authentication Service. Um protocolo de logon único que permite que os usuários se autentiquem uma vez e acessem múltiplos aplicativos. |
...
| Termo | Definição |
|-------|-----------|
| **Symfony** | O framework PHP no qual o Chamilo 2.0 é construído. O Symfony fornece roteamento, injeção de dependência, ORM (Doctrine), templating (Twig) e outras infraestruturas. |
...
| **Composer** | O gerenciador de dependências do PHP. Usado para instalar e atualizar as bibliotecas PHP do Chamilo. |
...
| **APCu** | Uma extensão do PHP que fornece um cache em memória de nível de usuário. Usado pelo Symfony para armazenar metadados e configurações em cache. |
