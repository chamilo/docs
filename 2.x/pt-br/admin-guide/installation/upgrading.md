...
A atualização do Chamilo 1.11.x para 2.x é uma **migração significativa**, não apenas uma simples atualização. O Chamilo 2.0 foi reconstruído no framework Symfony com um esquema de banco de dados reestruturado, nova API e organização de arquivos diferente. Planeje essa migração com cuidado e teste-a em um ambiente de teste antes de implementá-la em produção.
...
* **Agende atualizações durante janelas de manutenção** quando os usuários não estiverem utilizando ativamente a plataforma.
