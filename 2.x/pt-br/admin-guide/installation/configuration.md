...
Um arquivo padrão `.env.dist` é fornecido com o Chamilo e contém valores padrão documentados. Crie o arquivo `.env` (necessário para iniciar a instalação) para substituir os valores conforme o seu ambiente.
...
As configurações no nível do Symfony estão localizadas no diretório `config/`. Esses arquivos YAML controlam o comportamento do framework, definições de serviços e configurações específicas de pacotes.
...
O Symfony suporta configurações por ambiente. Arquivos em `config/packages/prod/` substituem os padrões quando `APP_ENV=prod`, e `config/packages/dev/` substituem quando `APP_ENV=dev`.

Por exemplo, `config/packages/prod/monolog.yaml` geralmente configura um registro de logs menos detalhado do que o equivalente para desenvolvimento.
