...
A ferramenta de pesquisa permite que você crie questionários para coletar feedback dos seus alunos. As pesquisas são úteis para avaliações de curso, análises de necessidades e enquetes de opinião.
...
* **Nota na ferramenta de avaliação** — Se o estado de resposta desta pesquisa será incluído na ferramenta de avaliação (boletim). Quem completar a pesquisa receberá 100%, os demais receberão 0%
* **Pesquisa pai** — Não é realmente usada no momento (funcionalidade legada)
* **Uma pergunta por página** — Estilo de apresentação das perguntas
* **Ativar modo de embaralhamento** — Se as perguntas serão embaralhadas
* **Mostrar número da pergunta** — Se os números das perguntas (gerados automaticamente) serão exibidos

## Adicionando Perguntas

Após criar a pesquisa, adicione perguntas:
...
1. Escolha o tipo de pergunta:
   ...
   * **Quebra de página** — Adicione quebras de página no fluxo de perguntas. Útil apenas se "Uma pergunta por página" **não** foi selecionada na etapa anterior
2. Configure o texto da pergunta e as opções de resposta
3. Salve

Cada pergunta pode ser marcada como obrigatória. Se não for marcada, pular qualquer pergunta será um comportamento aceitável.
