...
As avaliações (anteriormente *gradebook*) agregam pontuações de exercícios, tarefas e outras atividades avaliadas em uma visão unificada do desempenho de cada aluno. Elas também controlam a geração de certificados.
...
4. **Uma pontuação mínima por atividade** — Cada atividade no sistema de avaliações pode ter sua própria **Pontuação mínima**. Alunos que obtiverem pontuação abaixo desse mínimo em uma atividade-chave podem ser impedidos de alcançar os objetivos e obter o certificado, mesmo que sua pontuação total ponderada seja suficientemente alta.

As atividades podem ser de 2 tipos:
* **Atividade presencial** (ou atividade em sala de aula), onde as notas precisam ser importadas de alguma outra fonte
...
2. Escolha o tipo:
   ...
   * **Tarefa** — Vincule uma pasta de publicações de alunos
...
* **Exemplo**: Uma subcategoria "Tarefas de Casa" (peso: 30%) contendo cinco tarefas individuais, cada uma valendo 20% da subcategoria
...
* **Verifique regularmente** — Revise o sistema de avaliações periodicamente para garantir que todas as atividades estejam corretamente vinculadas e que as pontuações estejam sendo registradas