...
This command always documents the feature in **`3.x/en/`** — the active English source space (see
this repo's `CLAUDE.md`). `chamilo/chamilo-lms`'s `master` branch is what `3.x` documents.
...
- Either of the above followed by an explicit target, e.g. `... in admin-guide/platform-settings`
  or `... section: teacher-guide/creating-your-course` (both relative to `3.x/en/`)
...
- **target_section** (optional): if the text contains "in `<path>`" or "section: `<path>`", extract
  it as the user-mandated placement (relative to `3.x/en/`). Otherwise leave unset — you will
  propose a placement in Step 4.

---

## Step 2 — Locate the source

Prefer the local checkout at `/var/www/chamilo/master` (Chamilo LMS, `master` branch). Do not
`git pull`/`git fetch` without checking first whether the referenced commit(s) already exist
locally — a silent fetch can pull in unrelated upstream changes onto a checkout that may have
local state.
...
If `target_section` was given, validate it exists under `3.x/en/` (or is a sensible new file under
an existing section there) and use it — no need to ask.

Otherwise, inspect `3.x/en/SUMMARY.md` and the guides under `3.x/en/` (`admin-guide/`,
`teacher-guide/`, `developer-guide/`, `student-guide/`) to find the best-fitting existing section,
following the same reasoning as existing placements (e.g. audience — admin-only config vs.
teacher-facing course tool vs. developer API/architecture vs. learner-facing; and proximity — does
it belong as its own page, or as a subsection of an existing page). Prefer adding to an existing
page over creating a new one when the feature is a small addition to an existing tool/setting.

Present to the user:
- The proposed file (new or existing) and its position in `3.x/en/SUMMARY.md`.
...
or any other instance given instead.
Automate it with real Playwright, available via the `playwright` package already installed under
`/var/www/chamilo/master/node_modules` — run Node scripts from that directory so the module
resolves:
...
2. **Force English interface language** for that profile: go to `/account/edit` and set the
   **Langue/Language** field (`#profile_locale`) to `en_US`, *before* navigating to the feature —
   do this even if you believe it is already English, since the shared demo instance's state is not
   guaranteed between sessions (Chamilo's locale codes, e.g. `en_US`/`fr_FR`, are not the same as
   GitBook's language codes used elsewhere in this repo).
...
5. Save under `3.x/en/.gitbook/assets/` (in this repo) following the existing naming convention
   (`<guide-prefix>-<feature-slug>.png`, e.g. `admin-mcp-api-key.png`, `course-creation-form.png`).
6. **Record it in the catalogue**: add an entry to
   `3.x/en/.gitbook/assets/screenshot-catalogue.yaml` (schema documented in that file's header) —
   filename, the doc page (Step 6 below), the URL, the role, and any steps beyond plain navigation.
   This is what lets `/localize-screenshots` reproduce the same shot in another language without
   re-deriving all of this from scratch. Do this for every screenshot, even ones you consider
   simple — a "steps: null, url: /admin" entry is still far cheaper to reuse than starting from zero.
...
- Explain any gating platform setting by name (`` `variable_name` ``) and where to find it in
  **Administration > Configuration settings > \<category>**, matching the phrasing used in
  `3.x/en/admin-guide/platform-settings/*.md`.

Then wire the page in:
- `3.x/en/SUMMARY.md` — insert the new entry in the position agreed in Step 4.
...
- Confirm every referenced icon/screenshot file actually exists at the path used, under `3.x/en/`.
- Confirm the `3.x/en/SUMMARY.md` entry's path matches the file's real path exactly.
...
- Files created/edited, with paths (relative to the repo root, i.e. starting with `3.x/en/`).
- Screenshot(s) taken and where they were saved, and whether the catalogue was updated — or why
  none were needed.
- The commit prefix this change should use if/when committed (per
  `3.x/en/developer-guide/contributing/git-workflow.md` — `Documentation:` for docs-only changes,
  or the singular tool-name prefix if one clearly fits).
- Any part of the feature you could not verify (e.g. a setting whose effect you inferred from code
  ...
  already used in `3.x/en/admin-guide/platform-settings/*.md`.
- This English-only page is now out of sync with the other `3.x/<lang>/` spaces — mention that
  translating it (via `scripts/translate-docs.php --version 3.x`) and localizing its screenshot(s)
  (via `/localize-screenshots`) are follow-up steps, not something this command does itself.
