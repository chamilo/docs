...
Kurswerkzeug-Plugins fügen der Kursstartseite neue Werkzeuge hinzu, neben den integrierten Werkzeugen wie Dokumente, Übungen und Foren.

## Funktionsweise von Kurswerkzeug-Plugins

Wenn sich ein Plugin als Kurswerkzeug registriert:

1. Es erscheint im Werkzeugraster der Kursstartseite
2. Lehrende können es wie jedes andere Werkzeug ein- und ausblenden
3. Ein Klick auf das Werkzeug öffnet die Oberfläche des Plugins im Kurskontext

## Registrierung als Kurswerkzeug

Setzen Sie in Ihrer Plugin-Klasse `$isCoursePlugin = true`. Um automatisch ein Werkzeugsymbol auf der Kursstartseite hinzuzufügen, setzen Sie außerdem `$addCourseTool = true`:
...
Definieren Sie konfigurationsbezogene Felder auf Kursebene über die Eigenschaft `$course_settings`:
...
Diese erscheinen im Kurs-Einstellungsbereich und können durch Überschreiben von `validateCourseSetting(string $variable)` validiert werden (geben Sie `false` zurück, um einen Wert abzulehnen) oder über `course_settings_updated(array $values)` verarbeitet werden.
...
* **`CToolStateProvider`** (`src/CoreBundle/State/CToolStateProvider.php`) — Ermittelt, welche Werkzeuge (einschließlich Plugin-Werkzeuge) auf der Kursstartseite erscheinen
* Das Werkzeug erscheint in der `CTool`-Sammlung des Kurses

## Kurskontext

Wenn ein Lernender auf das Werkzeug Ihres Plugins klickt, wird Ihr Plugin-Code im Kurskontext ausgeführt. Sie können zugreifen auf:
...
* Plugin-Einstellungen auf Kursebene
