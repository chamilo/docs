...
| `Controller/` | Endpoints de inicialização e configuração LTI |
...
O Symfony mescla automaticamente os arquivos base `packages/*.yaml` com aqueles no subdiretório de ambiente correspondente (`dev/`, `prod/` ou `test/`), de modo que os arquivos específicos de ambiente precisam apenas sobrescrever os valores que diferem.

## Testes (`tests/`)

`tests/` **não está incluído nos downloads empacotados do Chamilo** (ZIPs/tarballs de release) — é removido porque não tem finalidade em tempo de execução e alguns de seus scripts poderiam representar um risco se deixados em um servidor de produção. Está presente apenas quando o projeto é obtido via `git clone`.
...
| Diretório | Conteúdo |
|-----------|----------|
| `CoreBundle/` | Testes PHPUnit espelhando `src/CoreBundle/`: `Api/`, `ApiResource/`, `Command/`, `Controller/`, `DataFixtures/`, `Entity/`, `Event/`, `EventListener/`, `Filter/`, `fixtures/`, `Helpers/`, `Mcp/`, `Migrations/`, `Repository/`, `Security/`, `Serializer/`, `Service/`, `Settings/`, `State/`, `Tool/`, `Traits/`, `Twig/` |
| `CourseBundle/` | Testes PHPUnit espelhando `src/CourseBundle/`: `Api/`, `Component/CourseCopy/`, `Repository/`, `Settings/` |
| `datafiller/` | Scripts que preenchem uma instalação de teste com conteúdo de demonstração: `data_courses.php`, `data_users.php`, `fill_courses.php`, `fill_users.php`, `fill_many_users.php`, `fill_whoisonline.php`, `generate_users.php`, `fill_all.php` (executa os demais), além de `images/` e um grande exemplo CSV de importação de usuários |
| `history/` | Instantâneos que documentam a estrutura do Chamilo em releases anteriores (`1.8.8.2`, `1.9.0`, `1.10.0`, `1.11.0`, `2.0`) |
| `phpstan/` | `doctrine-orm-bootstrap.php`, carregado pelo PHPStan ao analisar código Doctrine ORM |
| `playwright/` | Testes de ponta a ponta no navegador: `features/*.feature` (cenários Gherkin executados via [playwright-bdd](https://vitalets.github.io/playwright-bdd/)), `steps/common.steps.ts` (definições de passos em TypeScript), `fixtures/` (arquivos de teste, p. ex. planilhas), `scripts/check-results.mjs`, `playwright.config.ts` e a saída gerada `.features-gen/`. Substitui a antiga suíte Behat, cujos cenários permanecem no histórico do git para referência |
| `procedures/` | Planilhas (atualmente `spanish/`) usadas como base de checklist para revisão manual de qualidade das funcionalidades |
| `scripts/` | Scripts pontuais de manutenção/correção/migração para portais Chamilo existentes (em sua maioria voltados a versões mais antigas), além das subpastas `git-hooks/`, `img/`, `lang/` e `packaging/` |

Consulte [Testes](../contributing/testing.md) para saber como configurar o banco de dados de testes e executar as suítes PHPUnit e Playwright.

## Configuração de build

| Arquivo | Finalidade |
|------|---------|
| `webpack.config.js` | Configuração do Webpack Encore (entries, loaders, plugins) |
...
Todos os arquivos ficam na raiz do projeto. Os plugins PostCSS (Tailwind + Autoprefixer) são configurados inline dentro de `webpack.config.js` via `enablePostCssLoader()` — não há um `postcss.config.js` independente. `webpack.config.js` lê `tailwind.config.js` indiretamente por meio do PostCSS, de modo que alterações nas seções `content` ou `theme` do Tailwind passam a valer na próxima execução de `yarn encore dev` / `yarn encore production`.

## Pontos de entrada do Webpack

O build produz estes bundles:
...
O Tailwind é integrado via PostCSS. `assets/css/_tailwind.scss` emite as camadas base, component e utility; `assets/css/app.scss` o importa primeiro para que as utilities do Tailwind fiquem disponíveis em todos os demais parciais. A configuração do Tailwind — caminhos de conteúdo para purging, extensões de tema e plugins — fica em `tailwind.config.js` na raiz do projeto (`/var/www/chamilo/tailwind.config.js`).

Classes utilitárias e de componente personalizadas definidas com `@layer` (visíveis em `app.scss`) seguem a convenção de camadas do Tailwind, de modo que as classes definidas pelo usuário respeitem as mesmas regras de especificidade das utilities geradas.

### Temas de cores

O Chamilo oferece um sistema de temas de cores configurável diretamente na interface de administração (**Admin > Color Themes**). Cada tema salvo grava seus arquivos em um diretório dedicado em `var/themes/`:
...
`colors.css` define propriedades personalizadas de CSS como tripletos de canais RGB separados por espaço, em vez de valores `rgb()`, o que permite ao Tailwind compor variantes de opacidade (por exemplo, `bg-primary/50`) sem configuração adicional:
...
A camada de tema fica acima do pacote compilado Tailwind/SCSS: o navegador carrega `colors.css` depois da folha de estilos principal, de modo que as alterações de tema entram em vigor imediatamente, sem etapa de build.