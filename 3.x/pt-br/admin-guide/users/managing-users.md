...
Esta página aborda as tarefas do dia a dia de criação, edição e gerenciamento de contas de usuários.
...
Clique em **Exportar usuários** para baixar a lista de usuários como arquivo CSV ou XML. Você pode filtrar quais usuários exportar por função, data de cadastro ou outros critérios.
...
Ao excluir usuários (geralmente professores) que criaram conteúdo na plataforma, o sistema pode impedir a exclusão permanente e exibir uma mensagem de aviso explicando que o usuário ainda está vinculado a alguns dos recursos. Se você confirmar a exclusão, o sistema não excluirá o conteúdo em si, mas o vinculará a um usuário neutro (chamamos de "usuário Fallback") por razões de consistência dos dados.

Para evitar isso, verifique os detalhes do usuário, exclua cada um de seus cursos um a um e, em seguida, exclua o usuário.
...
* **Defina datas de expiração** — Para usuários temporários (participantes de oficinas, usuários em período de teste), defina uma data de expiração para desativar automaticamente as contas
