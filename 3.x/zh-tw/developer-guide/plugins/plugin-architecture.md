...
外掛存放於 `public/plugin/`。每個外掛擁有自己的目錄：
...
每個外掛皆繼承 `Plugin` 基底類別（`public/main/inc/lib/plugin.class.php`），並遵循單例模式：
...
| 屬性 | 型別 | 作用 |
|----------|------|--------|
| `$isCoursePlugin` | bool | 將此外掛註冊為課程工具 |
| `$isAdminPlugin` | bool | 新增管理介面頁面 |
| `$isMailPlugin` | bool | 與郵件系統整合 |
| `$addCourseTool` | bool | 於課程首頁新增圖示 |
| `$course_settings` | array | 定義各課程的設定欄位 |

## 外掛生命週期

1. **安裝** — 管理員啟用外掛，並執行 `install.php`
2. **設定** — 設定項目於管理面板中定義與管理；儲存於 `access_url_rel_plugin`（支援多租戶）
3. **執行** — 外掛將內容注入顯示區域，或對平台事件作出反應
4. **停用** — 外掛被停用，但其資料予以保留
5. **解除安裝** — 執行 `uninstall.php` 以清理資料與資料表

## 顯示區域

外掛透過覆寫 `renderRegion()`，將 HTML 注入 Vue 前端的 18 個預定義區域：
...
置於 `src/EventSubscriber/` 且檔名以 `EventSubscriber.php` 結尾的檔案，會透過 `PluginEventSubscriberPass` 自動註冊。它們實作 `EventSubscriberInterface`，並對 `src/CoreBundle/Event/Events.php` 中定義的事件作出反應。

由於外掛類別（`MyPluginPlugin`）並非 Symfony 服務，無法自動注入訂閱者的建構函式。請改用 `create()` 單例：
...
置於 `src/Entity/` 的 Doctrine 實體會由 `PluginEntityPass` 自動發現。請使用 PHP 8 屬性進行對應。命名空間必須遵循 `Chamilo\PluginBundle\{PluginName}`。請使用獨特的資料表名稱前綴（例如 `my_plugin_*`）以避免衝突。

### PluginHelper 服務

若要從核心 Symfony 服務存取外掛狀態，請注入 `PluginHelper`，而非直接實例外掛類別：
...
| 方法 | 用途 |
|--------|---------|
| `isPluginEnabled(string $name): bool` | 檢查外掛是否已安裝，且對目前存取 URL 為啟用狀態 |
| `loadLegacyPlugin(string $name): ?object` | 實例化並回傳外掛單例 |
| `getPluginSetting(string $name, string $key): mixed` | 讀取單一外掛設定值 |
| `getPluginOverrides(string $name): array` | 取得外掛的 `plugin.yaml` 覆寫（預設值 + 特定存取 URL） |
...
| `public/main/inc/lib/plugin.lib.php` | 外掛管理員 |
| `src/CoreBundle/Entity/Plugin.php` | Plugin Doctrine 實體 |
