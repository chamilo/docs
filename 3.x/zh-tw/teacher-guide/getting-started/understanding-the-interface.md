...
Chamilo 3.0 具備簡潔、現代化的介面，旨在讓導覽保持簡單。本頁將詳細說明介面的各個部分。
...
左側側邊欄是主要導覽。可將其收合，以便為內容區騰出更多空間。點選其右緣的切換箭頭即可展開或收合。Chamilo 會記住您的偏好。
...
畫面中央區域會顯示目前頁面的內容。頂端常會看到 **麵包屑路徑**，顯示您在平台中的目前位置（例如：首頁 > 搖滾音樂 > 文件）。可使用麵包屑返回上層頁面。

## 課程首頁

進入課程後，您會看到 **課程首頁**。詳細說明見 [建立您的課程](../creating-your-course/) 一節，以下為快速概覽：

* **課程標題** — 醒目顯示於頂端
* **課程簡介** — 可選的富文字說明，您可以編輯
* **工具格線** — 以圖示格線呈現課程工具（文件、練習、論壇等）
...
此功能在 Chamilo 3.0 中仍屬實驗性質，尚未完全完備，但我們正嘗試在介面中的所有按鈕與操作圖示上套用下列規則：

* **綠色** 用於建立類操作。包含新增、建立、匯入、評分、儲存與複製內容。
* **藍色** 用於檢視類操作。包含匯出、檢視、在清單或詳細檢視中預覽、搜尋與下載。
* **橘色** 用於編輯類操作。包含編輯、移動、設定、啟用／停用、隱藏與顯示。
* **紅色** 用於刪除／移除類操作。包含刪除、移除、取消訂閱。
* **灰色** 用於取消類操作。僅維持現狀、不做變更。
...
這表示您與學習者可從手機、平板或電腦存取平台，但介面體驗可能略有不同。